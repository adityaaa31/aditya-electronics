...
async function startServer() {
  ...
  const transporter = nodemailer.createTransport({
    service: 'gmail',
    auth: {
      user: process.env.EMAIL_USER || 'storeadityaelectronics@gmail.com',
      pass: process.env.EMAIL_PASS || 'Satish@3122Soni'
    }
  });
  ...
  app.post("/api/auth/register", async (req, res) => {
    ...
    try {
      ...
      const token = jwt.sign({ id: result.lastInsertRowid, role: "customer" }, JWT_SECRET);
