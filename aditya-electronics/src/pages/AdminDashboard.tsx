...
const Overview = () => {
  ...
  React.useEffect(() => {
    api.get('/products').then(res => setStats(prev => ({ ...prev, products: res.data.length })));
    api.get('/admin/bookings').then(res => setStats(prev => ({ ...prev, bookings: res.data.length })));
    api.get('/admin/db-info').then(res => setDbInfo(res.data));
  }, []);
...
const ManageProducts = () => {
  ...
  const [newProduct, setNewProduct] = React.useState({ name: '', price: '', description: '', category_id: '', warranty: '' });
  ...
  const fetchCategories = () => api.get('/categories').then(res => {
    ...
    if (res.data.length > 0) setNewProduct(prev => ({ ...prev, category_id: res.data[0].id.toString() }));
  });
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowAdd(false);
      setNewProduct({ name: '', price: '', description: '', category_id: '', warranty: '' });
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append('warranty', editingProduct.warranty);
        ...
        <div className="bg-white dark:bg-gray-900 p-8 rounded-3xl shadow-lg border border-gray-100 dark:border-gray-800 mb-8">
          <form onSubmit={handleAdd} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Product Images</label>
              <input 
                type="file" 
                multiple 
                accept="image/*"
                ...
                className="w-full p-4 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl outline-none focus:ring-2 focus:ring-red-500 dark:text-white"
              />
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">You can select multiple images.</p>
            </div>
      ...
      {editingProduct && (
        <div className="bg-white dark:bg-gray-900 p-8 rounded-3xl shadow-lg border border-gray-100 dark:border-gray-800 mb-8">
          ...
          <form onSubmit={handleUpdate} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            />
            <div className="md:col-span-2">
              <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Add More Images</label>
              <input 
                type="file" 
                multiple 
                accept="image/*"
...
const ManageServices = () => {
  ...
  const [showAdd, setShowAdd] = React.useState(false);
  const [newService, setNewService] = React.useState({ name: '', price: '', description: '', warranty: '' });
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append('warranty', newService.warranty);
      ...
      setShowAdd(false);
      setNewService({ name: '', price: '', description: '', warranty: '' });
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append('warranty', editingService.warranty);
      if (editingService.image_url) formData.append('image_url', editingService.image_url);
      if (selectedFile) formData.append('image', selectedFile);
